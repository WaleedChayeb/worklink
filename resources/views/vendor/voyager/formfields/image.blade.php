@if(isset($dataTypeContent->{$row->field}))
    <div data-field-name="{{ $row->field }}">
        ...
        <img src="@if( !filter_var($dataTypeContent->{$row->field}, FILTER_VALIDATE_URL)){{ GenericHelper::getStorageAssetLink( $dataTypeContent->{$row->field} ) }}@else{{ $dataTypeContent->{$row->field} }}@endif"
